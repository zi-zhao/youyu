<template>
    <div class="cbutton">
        <span class="tatitle">TA的分享专栏</span>
    </div>
...
<script setup>
    ...
    async function fetchArticleColumn(currentPage) {
        try {
            ...
            const filteredColumns = response.data.data.records.filter(shareColumn => shareColumn.visibility === 1); // 只保留公共可见的分享专栏

            const updates = filteredColumns.map(async (column) => {
                const coverPicture = await getColumnPictureFromCOS(column.id);
                return {
                ...column,
                coverPicture
                };
