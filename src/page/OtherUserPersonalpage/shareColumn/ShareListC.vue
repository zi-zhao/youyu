<template>
  <div class="cbutton">
    <span class="tatitle">TA的分享</span>
  </div>
  <div class="sharepage1" v-infinite-scroll="load" infinite-scroll-distance="100" infinite-scroll-immediate="false">
    <div class="show" v-for="(data, index) in articles" :key="index" @click="goToDetail(item)">
...
<script setup>
  ...
  const id = store.state.columnShareTest;
  const articles = ref([]);
  const shareId = ref();
  ...
  async function fetchArticleColumn(currentPage) {
    try {
      ...
      if (response.data.code === 1 && response.data.data.records.length > 0) {
        const filteredArticles = response.data.data.records.filter(article => article.access === 0); // 只保留公共可见的分享
        const updates = filteredArticles.map(async (share) => {
        ...
        console.error("获取专栏的收藏列表失败");
        ElMessage.error("获取专栏的收藏列表失败");
      }
    } catch (error) {
      console.error("获取专栏的收藏列表失败", error);
      ElMessage.error('获取专栏的收藏列表失败');
    }
  }
