...
<script setup>
    ...
    async function fetchArticleColumn(currentPage) {
        try {
            ...
            const filteredColumns = response.data.data.records.filter(articleColumn => articleColumn.visibility === 1); // 只保留公共可见的文章
            const updates = filteredColumns.map(async (record) => {
            ...
            articleColumnData.value = [...articleColumnData.value, ...updatedColumns];
