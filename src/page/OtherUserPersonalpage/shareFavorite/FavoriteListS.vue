<template>
    <div class="cbutton">
        <span class="tatitle">TA的分享收藏</span>
    </div>
    <div class="hh" v-infinite-scroll="load" infinite-scroll-distance="100" infinite-scroll-immediate="false">
      <div class="show" v-for="data in shareFavoriteData" :key="data.id">
        <div style="display: flex;flex-direction: column;justify-content: space-between;height: 100%;">
            <span class="title">{{ data.name }}</span>
            <span class="content multi-line-ellipsis-2 ">{{ data.profile }}</span>
            <div class="more">
                <span class="time">创建时间{{ data.createTime }}</span>
                <span class="time">分享数量 {{ data.number }}</span>
                <span class="time">获赞数 {{ data.likeCount }}</span>
                <span class="text"  @click="setActive(data)">查看分享内容</span>
            </div>
        </div>
        <img :src="data.coverPicture" alt="" class="carepic">
      </div>
      <el-empty 
          v-if="shareFavoriteData.length === 0 || !shareFavoriteData" 
          :image-size="200" 
          description="他还没有分享收藏夹呢，逛逛别的吧！"
      />
    </div>
</template>
  ...
  async function load() {
    ...
    async function fetchArticleColumn(currentPage) {
        try {
            ...
            const filteredFavorites = response.data.data.records.filter(shareFavorite => shareFavorite.visibility === 1); // 只保留公共可见的分享
            ...
            shareFavoriteData.value = [...shareFavoriteData.value, ...updatedFavorites];
