<template>
    <div class="cbutton">
        <span class="tatitle">我的文章收藏</span>
    </div>
        ...
        <div class="show" v-for="item in articleFavoriteData" :key="item.id">
            <div style="display: flex;flex-direction: column;justify-content: space-between;height: 100%;">
                ...
                <div class="more">
                    ...
                    <span class="text"  @click="setActive(item)">进入收藏夹</span>
                </div>
            </div>
...
<script setup>
  ...
  async function load() {
      ...
      try {
        ...
        await getFavoriteArticle(currentPage.value);
  ...
  async function getFavoriteArticle(currentPage) {
  try {
    const response = await axios.get("/favorite", {
      params: {
        type: 0,
        page: currentPage,
        pageSize: 10, // 注意这里修正了拼写错误
    ...
    if (response.data.code === 1 && response.data.data.records.length > 0) {
      ...
      articleFavoriteData.value = updatedFavorites;
  ...
  onMounted(() => {
    getFavoriteArticle(currentPage.value);
  });
</script>
