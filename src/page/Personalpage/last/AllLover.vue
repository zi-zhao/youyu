...
async function getMyFocusNote(currentPage) {
  try {
    ...
    if (response.data.code === 1 && response.data.data.records.length > 0) {
      ...
      const updatesAsync = updates.map(async (record) => {
        const newAvatar = await getUsersAvatarFromCOS(record.fanId);
        const createTime = convertDateFormat(record.createTime);
        const verified = await fetchUserInformation(record.userId);
        const focusStatus = deterFocus(record);
        return {
          ...record,
          createTime,
          verified,
