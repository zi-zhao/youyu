...
//获取用户近7天转发数据
async function getSevenForwardData() {
  try {
    const response = await axios.get(`/articledata/forward`)
    if(response.data.code === 1){
        userRecentCreateData.value = response.data.data;
        // ElMessage.success('获取用户近7天转发数据成功');
    }else if(response.data.code === 0 && response.data.msg){
        ElMessage.error(response.data.msg.toString());
    }else{
        console.error("获取用户近7天转发数据成功");
        ElMessage.error("获取用户近7天转发数据成功");
    }
  } catch (error) {
    console.error("获取用户近7天转发数据成功", error); 
    ElMessage.error("获取用户近7天转发数据成功");
  }
}
