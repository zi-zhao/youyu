<template>
  <div class="middle">
    ...
    <!-- 搜索框样式 -->
  <div
    style="display: flex; align-items: center; width: 300px; height: 35px; border: 1px solid #ccc; border-radius: 4px;">
  ...
  <div class="content-container" v-infinite-scroll="load" infinite-scroll-distance="100" infinite-scroll-immediate="false">
    <div class="content-box underline"  v-for="item in articleColumnGroups" :key="item.id" style="display: flex; flex-direction: row; gap: 1rem;">
      ...
      <div style="display: flex;justify-content: space-evenly; flex: 25%;">
          ...
          <el-link type="danger"  @click="deleteColumnArticle(item.id)" >删除</el-link>
      </div>
...
<!-- 我叫不出名的组件 -->
  <div v-if="dialogFormVisible2">
    <EditDialog v-model="dialogFormVisible2" ClassifyName = "文章专栏" :Id = Id> </EditDialog>
  </div>
...
const manage = (id,name) => {
  emits('manage',{ title: '文章专栏',id: id ,name:name});
};
...
//点击删除的弹框
const deleteColumnArticle = async (id) => {
  try {
...
async function handleDelete(id) {
  ...
  try {
        ...
        if (response.data && response.data.code === 1) {
            ...
            console.error('删除文章专栏失败', response.data.msg);
            ElMessage.error('删除文章专栏失败');
        }
  } catch (error) {
      console.error("删除文章专栏接口失败", error); 
      ElMessage.error('删除文章专栏接口失败');
  }
}
...
async function load() {
    ...
    try {
      ...
      await getColumnArticleList(currentPage.value);
...
//获取文章专栏列表
async function getColumnArticleList(currentPage) {
  try {
    ...
    if(response.data.data.records.length > 0 && response.data.code === 1){
      ...
      articleColumnGroups.value = [...articleColumnGroups.value, ...updatedArticles];
...
onMounted(() => {
  getColumnArticleList(currentPage.value);
})
