<template>
  ...
  <div class="content-container" v-infinite-scroll="load" infinite-scroll-distance="100" infinite-scroll-immediate="false">
    <div class="content-box underline"  v-for="item in articleFavoriteGroups" :key="item.id" style="display: flex; flex-direction: row; gap: 1rem;">
      ...
      <div style="display: flex;justify-content: space-evenly; flex: 25%;">
          ...
          <el-link type="danger"  @click="deleteFavoriteArticle(item.id)" >删除</el-link>
      </div>
    ...
    <!-- 当专栏为空时的显示信息 -->
    <el-empty 
      v-if="!articleFavoriteGroups || articleFavoriteGroups.length === 0" 
      :image-size="200" 
    ...
    <div v-if="dialogFormVisible3">
        <EditDialog v-model="dialogFormVisible3" ClassifyName = "文章收藏" :Id = Id> </EditDialog>
    </div>
</template>
<script setup>
...
const keyword = ref('')
const articleFavoriteGroups = ref([])
...
const manage = (id,name) => {
  emits('manage',{ title: '文章收藏',id: id,name:name });
};
...
//点击删除的弹框
const deleteFavoriteArticle = async (id) => {
  try {
...
async function handleDelete(id) {
  ...
  try {
        ...
        if (response.data.code === 1) {
            ...
            console.error('删除文章收藏失败', response.data.msg);
            ElMessage.error('删除文章收藏失败');
        }
  } catch (error) {
      console.error("删除文章收藏失败", error); 
      ElMessage.error('删除文章收藏失败');
  }
}
...
async function load() {
    ...
    try {
      ...
      await getFavoriteArticleList(currentPage.value);
...
//获取文章收藏列表
async function getFavoriteArticleList(currentPage) {
  try {
    ...
    if(response.data.data.records.length > 0 && response.data.code === 1){
      ...
      articleFavoriteGroups.value = [...articleFavoriteGroups.value, ...updatedArticles];
...
onMounted(() => {
    getFavoriteArticleList(currentPage.value);
})
