    ...
    <div class="container1">
        <div class="page">
            ...
            <div class="content-container"  v-infinite-scroll="load" infinite-scroll-distance="100" infinite-scroll-immediate="false">
                <div class="content-box underline"  v-for="item in shareColumnGroups" :key="item.id">
                ...
                <el-empty 
                    ...
                    :image-size="200" 
                    description="您的专栏里面还没有分享呢，快去创作吧！"
                />
            </div>
        </div>
...
const dialogFormVisible = ref(false)//控制cfdialog弹窗（显示分享专栏列表）是否弹出
const columnId = ref();//用户选择的ID
const tempcolumnId = ref()//存放用户临时选择的分享ID
...
const back = () => {
    emits("switchToComponent",{title:"分享专栏"});
};
...
async function handleDelete() {
    try {
        const response = await axios.delete(`/column/share`,{columnId:props.Id,contentId: tempcolumnId.value});
        if (response.data && response.data.code === 1) {
            console.success('删除专栏分享成功');
        } else if(response.data.code === 0 && response.data.msg){
            ElMessage.error(response.data.msg.toString());
        }else {
            console.error('删除专栏分享失败', response.data.msg);
            ElMessage.error('删除专栏分享失败');
        }
    } catch (error) {
        console.error("删除专栏分享失败", error); 
        ElMessage.error('删除专栏分享失败');
    }
}
...
async function handleInsert() {
    try {
        const response = await axios.post(`/column/share`,{columnId:columnId.value,contentId: tempcolumnId.value});
        if (response.data && response.data.code === 1) {
            ElMessage.success('移动分享成功');
        } else if(response.data.code === 0 && response.data.msg){
            ElMessage.error(response.data.msg.toString());
        }else {
            console.error('增添专栏分享失败', response.data.msg);
            ElMessage.error('增添专栏分享失败');
        }
    } catch (error) {
        console.error("增添专栏分享失败", error); 
        ElMessage.error('增添专栏分享失败');
    }
}
...
async function getArticleList(currentPage) {
    try {
    const response = await axios.get('/column/share',{
        params: {
            id:props.Id,
            page:currentPage,
            pageSize:10,
    ...
    if (response.data.code === 1 && response.data.data.records.length > 0) {
      ...
      shareColumnGroups.value = [...shareColumnGroups.value, ...updatedRecords];
